...
unsigned int ngl_glvpx = 512, ngl_glvpy = 512; // Размер окна контекста TinyGL

int ngl_glrowalignment = 4;

GLContext *ngl_tinygl_context = 0;
static ZBuffer *ngl_tinygl_zb;
static int NewTinyGLResizeViewport(GLContext *c, int *xsize, int *ysize);
...
N_API bool N_APIENTRY_EXPORT nglInit(void)
{
	...
	// Инициализация Z-буфера TinyGL
	ngl_tinygl_zb = ZB_open(ngl_winx, ngl_winy, ZB_MODE_RGBA, 0, NULL, NULL, NULL);
	if(ngl_tinygl_zb == NULL) {
		ngl_ea->nlAddTab(-1); ngl_ea->nlPrint(LOG_FDEBUGFORMAT, F_NGLINIT, ERR_FAILEDTOINITIALIZEZBUFFER);
		return false;
	}

	// Инициализация TinyGL
	glInit(ngl_tinygl_zb);
	ngl_tinygl_context = gl_get_context();
	ngl_tinygl_context->gl_resize_viewport = NewTinyGLResizeViewport;
	ngl_tinygl_context->viewport.xsize = -1;
	ngl_tinygl_context->viewport.ysize = -1;
...
/*
	Функция	: NewTinyGLResizeViewport

	Описание: Изменяет размер вьюпорта, возвращает изменённые значения вьюпорта в параметрах.
		В случае ошибки возвращает -1, иначе 0.

	История	: 09.05.14	Создан
			21.04.16 Переписана снуля, чтобы избавиться от кода tinygl в файле
				(не хочу вставлять туда текст лицензии, он большой)

*/
static int NewTinyGLResizeViewport(GLContext *c, int *xsize, int *ysize)
{	
	// Как я понял, размер вьюпорта должен делиться на 4
	ngl_glvpx = (*xsize+3)&(~3);
	if(!ngl_glvpx) ngl_glvpx = 4;
	ngl_glvpy = (*ysize+3)&(~3);
	if(!ngl_glvpy) ngl_glvpy = 4;
	
	ZB_resize(c->zb, NULL, ngl_glvpx, ngl_glvpy);
