...
static void nglUpdateClippingRegion(void)
{
	if(ngl_winclippingregion) {
		...
		if(ngl_isinit) {
			...
			sglSetClippingRegion(ngl_windpix*ngl_winclippingregion_sx/100, ngl_windpiy*ngl_winclippingregion_sy/100, ngl_windpix*ngl_winclippingregion_ex/100, ngl_windpiy*ngl_winclippingregion_ey/100);
		}
	} else {
		...
		if(ngl_isinit) {
			...
			// Код обновления региона отрисовки графического API
			sglSetClippingRegion(0, 0, ngl_windpix*ngl_winx/100, ngl_windpiy*ngl_winy/100);
		}
	}
}
