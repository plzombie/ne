...
nal_buffer_type *nal_buffers; // Буферы для хранения аудио
unsigned int nal_maxbuffers = 0;

nal_source_type *nal_sources;
unsigned int nal_maxsources = 0;

nal_audiostream_type *nal_audiostreams;
unsigned int nal_maxaudiostreams = 0;

unsigned int nal_task;
...
*/
extern "C" N_API unsigned int N_APIENTRY_EXPORT nalCreateBuffer(void)
{
	unsigned int i;
	...
	if(i == nal_maxbuffers) {
		...
		_nal_buffers = (nal_buffer_type *)nal_ea->nReallocMemory(nal_buffers, (nal_maxbuffers+1024)*sizeof(nal_buffer_type));
...
*/
extern "C" N_API bool N_APIENTRY_EXPORT nalDestroyAllBuffers(void)
{
	bool success = true;
...
*/
extern "C" N_API bool N_APIENTRY_EXPORT nalDestroyBuffer(unsigned int id)
{
...
*/
extern "C" N_API bool N_APIENTRY_EXPORT nalLoadBuffer(unsigned int id, unsigned int nos, unsigned int noc, unsigned int freq, unsigned int sf, unsigned int bps, void *buf)
{
	bool success = true;
	...
	if(success) {
		nal_buffers[id-1].bufdatalen = nos*noc*bps;

		if(nal_buffers[id-1].bufdatalen) {
			nal_buffers[id-1].bufdata = (unsigned char *)malloc(nal_buffers[id-1].bufdatalen);

			if(nal_buffers[id-1].bufdata) {
				memcpy(nal_buffers[id-1].bufdata, buf, nal_buffers[id-1].bufdatalen);

				memset(&(nal_buffers[id-1].wavfmt), 0, sizeof(WAVEFORMATEX));

				nal_buffers[id-1].wavfmt.wFormatTag = 1;
				nal_buffers[id-1].wavfmt.nChannels = noc;
				nal_buffers[id-1].wavfmt.nSamplesPerSec = freq;
				nal_buffers[id-1].wavfmt.nAvgBytesPerSec = freq*noc*bps;
				nal_buffers[id-1].wavfmt.nBlockAlign = 4;
				nal_buffers[id-1].wavfmt.wBitsPerSample = bps*8;
				nal_buffers[id-1].wavfmt.cbSize = sizeof(WAVEFORMATEX);
...
*/
extern "C" N_API bool N_APIENTRY_EXPORT nalUnloadBuffer(unsigned int id)
{
	if(!nal_isinit || id > nal_maxbuffers || id == 0) return false;
...
static bool nalSubmitBufferToSource(unsigned int srcid)
{
	memset(&(nal_sources[srcid].xaudio2buffer), 0, sizeof(XAUDIO2_BUFFER));

	nal_sources[srcid].xaudio2buffer.AudioBytes = nal_buffers[nal_sources[srcid].bufid-1].bufdatalen;
	nal_sources[srcid].xaudio2buffer.pAudioData = nal_buffers[nal_sources[srcid].bufid-1].bufdata;

	if(nal_sources[srcid].islooped)
		nal_sources[srcid].xaudio2buffer.LoopCount = XAUDIO2_LOOP_INFINITE;

	if(nal_sources[srcid].sourcevoice->SubmitSourceBuffer(&(nal_sources[srcid].xaudio2buffer)) == S_OK)
		return true;
	else
		return false;
}
...
*/
extern "C" N_API unsigned int N_APIENTRY_EXPORT nalCreateSource(unsigned int bufid,int loop)
{
	unsigned int i;
	...
	if(i == nal_maxsources) {
		...
		_nal_sources = (nal_source_type *)nal_ea->nReallocMemory(nal_sources, (nal_maxsources+1024)*sizeof(nal_source_type));
	...
	if(nal_xaudio2->CreateSourceVoice(&(nal_sources[i].sourcevoice), &(nal_buffers[bufid-1].wavfmt), 0, XAUDIO2_MAX_FREQ_RATIO, 0, 0, 0) != S_OK) {
		nal_ea->nlAddTab(-1); nal_ea->nlPrint(LOG_FDEBUGFORMAT5, F_NALCREATESOURCE, N_FALSE, N_ID, 0);
	...
	nal_sources[i].islooped = loop;

	if(!nalSubmitBufferToSource(i)) {
		nal_ea->nlAddTab(-1); nal_ea->nlPrint(LOG_FDEBUGFORMAT5, F_NALCREATESOURCE, N_FALSE, N_ID, 0);

		return 0;
	}

	nal_sources[i].ispaused = true;
...
*/
extern "C" N_API bool N_APIENTRY_EXPORT nalDestroyAllSources(void)
{
	if(!nal_isinit) return false;
...
*/
extern "C" N_API bool N_APIENTRY_EXPORT nalDestroySource(unsigned int id)
{
	...
	nal_sources[id-1].sourcevoice->Stop();
	nal_sources[id-1].sourcevoice->DestroyVoice();
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalPlaySource(unsigned int id)
{
	if(!nal_isinit || id > nal_maxsources || id == 0) return;

	if(!nal_sources[id-1].used) return;

	nal_sources[id-1].sourcevoice->Start();
}
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalPauseSource(unsigned int id)
{
	if(!nal_isinit || id > nal_maxsources || id == 0) return;
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalStopSource(unsigned int id)
{
	if(!nal_isinit || id > nal_maxsources || id == 0) return;
	...
	nal_sources[id-1].sourcevoice->Stop();
	wprintf(L"PlayBegin %u\n", nal_sources[id-1].xaudio2buffer.PlayBegin);
}
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalReplaySource(unsigned int id)
{
	if(!nal_isinit || id > nal_maxsources || id == 0) return;
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalGetSourceLoop(unsigned int id, int *loop)
{
	if(!nal_isinit || id > nal_maxsources || id == 0) return;

	if(!nal_sources[id-1].used) return;

	*loop = nal_sources[id-1].islooped;
}
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalSetSourceLoop(unsigned int id, int loop)
{
	if(!nal_isinit || id > nal_maxsources || id == 0) return;
	...
	if(loop) {
		...
		nal_sources[id-1].sourcevoice->ExitLoop();

		nal_sources[id-1].islooped = loop;
	}
}
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalGetSourceSecOffset(unsigned int id, unsigned int *offset)
{
	...
	nal_sources[id-1].sourcevoice->GetState(&voice_state);

	*offset = (unsigned int)(voice_state.SamplesPlayed/nal_buffers[nal_sources[id-1].bufid-1].wavfmt.nSamplesPerSec);
}
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalSetSourceSecOffset(unsigned int id, unsigned int offset)
{
	if(!nal_isinit || id > nal_maxsources || id == 0) return;
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalGetSourceLength(unsigned int id, unsigned int *length)
{
	...
	buff = nal_buffers[nal_sources[id-1].bufid-1].wavfmt.nSamplesPerSec; // Получение частоты аудиобуфера nal_buffers[nal_sources[id-1].bufid-1]
	bufb = nal_buffers[nal_sources[id-1].bufid-1].wavfmt.wBitsPerSample; // Получение количества бит на семпл аудиобуфера nal_buffers[nal_sources[id-1].bufid-1]
	bufc = nal_buffers[nal_sources[id-1].bufid-1].wavfmt.nChannels; // Получение каналов аудиобуфера nal_buffers[nal_sources[id-1].bufid-1]
	bufs = nal_sources[id-1].xaudio2buffer.AudioBytes; // Получение размера аудиобуфера nal_buffers[nal_sources[id-1].bufid-1] в байтах
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalGetSourceGain(unsigned int id, float *gain)
{
	if(!nal_isinit || id > nal_maxsources || id == 0) return;

	if(!nal_sources[id-1].used) return;

	nal_sources[id-1].sourcevoice->GetVolume(gain);
}
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalSetSourceGain(unsigned int id, float gain)
{
	if(!nal_isinit || id > nal_maxsources || id == 0) return;
	...
	nal_sources[id-1].sourcevoice->SetVolume(gain);
}
...
*/
extern "C" N_API unsigned int N_APIENTRY_EXPORT nalGetSourceStatus(unsigned int id)
{
...
*/
extern "C" N_API unsigned int N_APIENTRY_EXPORT nalCreateAudioStream(na_audiofile_type *aud, unsigned int files, na_audiofile_plg_read_type readaf, int loop)
{
	...
	if(i == nal_maxaudiostreams) {
		...
		_nal_audiostreams = (nal_audiostream_type *)nal_ea->nReallocMemory(nal_audiostreams, (nal_maxaudiostreams+1024)*sizeof(nal_audiostream_type));
	...
	nal_audiostreams[i].aud = (na_audiofile_type *)nal_ea->nAllocMemory(files*sizeof(na_audiofile_type));
	nal_audiostreams[i].wavFormat = (unsigned int *)nal_ea->nAllocMemory(files*sizeof(unsigned int));
	nal_audiostreams[i].secs = (unsigned int *)nal_ea->nAllocMemory(files*sizeof(int));
	...
	if(success) {
		nal_audiostreams[i].bufdata = (unsigned char *)nal_ea->nAllocMemory(maxbufsize);
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalPlayAudioStream(unsigned int id)
{
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalPauseAudioStream(unsigned int id)
{
	if(!nal_isinit || id > nal_maxaudiostreams || id == 0) return;
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalStopAudioStream(unsigned int id)
{
	if(!nal_isinit || id > nal_maxaudiostreams || id == 0) return;
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalReplayAudioStream(unsigned int id)
{
	if(!nal_isinit || id > nal_maxaudiostreams || id == 0) return;
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalGetAudioStreamLoop(unsigned int id, int *loop)
{
	if(!nal_isinit || id > nal_maxaudiostreams || id == 0) return;
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalSetAudioStreamLoop(unsigned int id, int loop)
{
	if(!nal_isinit || id > nal_maxaudiostreams || id == 0) return;
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalGetAudioStreamSecOffset(unsigned int id, unsigned int *offset, unsigned int *track)
{
	if(!nal_isinit || id > nal_maxaudiostreams || id == 0) return;
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalSetAudioStreamSecOffset(unsigned int id, unsigned int offset, unsigned int track)
{
	if(!nal_isinit || id > nal_maxaudiostreams || id == 0) return;
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalGetAudioStreamLength(unsigned int id, unsigned int *length, unsigned int *tracks)
{
	if(!nal_isinit || id > nal_maxaudiostreams || id == 0) return;
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalGetAudioStreamGain(unsigned int id, float *gain)
{
	if(!nal_isinit || id > nal_maxaudiostreams || id == 0) return;
...
*/
extern "C" N_API void N_APIENTRY_EXPORT nalSetAudioStreamGain(unsigned int id, float gain)
{
	if(!nal_isinit || id > nal_maxaudiostreams || id == 0) return;
...
*/
extern "C" N_API unsigned int N_APIENTRY_EXPORT nalGetAudioStreamStatus(unsigned int id)
{
	unsigned int status = NA_SOURCE_STOP;
...
*/
extern "C" N_API bool N_APIENTRY_EXPORT nalDestroyAudioStream(unsigned int id)
{
	int processed;
...
*/
extern "C" N_API bool N_APIENTRY_EXPORT nalDestroyAllAudioStreams(void)
{
	if(!nal_isinit) return false;
				...
				if(nal_audiostreams[i].mustreload && nal_audiostreams[i].status == NAL_SSOURCE_PLAY) {
					...
					while(processed){
						...
						if((nal_audiostreams[i].curpos+1) == nal_audiostreams[i].secs[curaf] && !nal_audiostreams[i].loop && (nal_audiostreams[i].curaf+1) == nal_audiostreams[i].noafs) {
							...
							// Удаление из очереди отработавшего буфера аудиопотока из очереди через audio api
