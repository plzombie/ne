...
void Magic_SetCamera(MAGIC_CAMERA* camera);
...
// eng: Returns the default camera of emitter that was set using the Magic Particles (Dev)
// rus: Возвращает камеру, которая была установлена для эмиттера в Magic Particles (Dev)
//int Magic_GetCameraDefault(HM_EMITTER hmEmitter, MAGIC_CAMERA* camera);
